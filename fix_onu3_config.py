"""Fix ONU 3 (ZTEGD1397E71) - add tcont, gemport, service-port, flow/vlan config."""
import paramiko
...
async def main():
    ...
    # First let's see ONU:1's full config for reference (the one that has service-port)
    proc.stdin.write('show running-config interface gpon-onu_1/9/2:1\n')
    out = await rp(proc, 10)
    print('=== ONU:1 config (reference) ===')
    print(re.sub(r'\x1b\[[0-9;]*[a-zA-Z]', '', out))

    # And the pon-onu-mng section for ONU:1
    proc.stdin.write('show running-config | begin pon-onu-mng gpon-onu_1/9/2:1\n')
    out = await rp(proc, 10)
    print('\n=== ONU:1 pon-onu-mng (reference) ===')
    print(re.sub(r'\x1b\[[0-9;]*[a-zA-Z]', '', out)[:3000])
...
_, stdout, _ = client.exec_command(
    "docker cp /tmp/fix3.py jtl_olt_api:/tmp/fix3.py && "
    "docker exec jtl_olt_api python /tmp/fix3.py 2>&1",
    timeout=120,
)
